...
enum {Version, Ants, Chan_max, Pols, Samplers, Macs, F_step, Clock,
      ...
      Dra, Ddec, Ra_app,Dec_app,Ra_mean,Dec_mean, Ra_date,Dec_date,
      MJD, Date_obs,Lta, Samp_num, Chan_num, Mac_num, Integ, 
      Mac_mode, Fft_mode, Dpc_mux, Clk_sel, Net_sign,Vars} VarType;
char *varname[Vars] =
     {"VERSION","ANTS","CHAN_MAX","POLS","F_ENABLE","X_ENABLE","F_STEP","CLOCK",
      "STA","STATIME","T_UNIT","RF","FIRST_LO","BB_LO","ANTMASK","BANDMASK",
      "MODE","SEQ","OBJECT","OBSERVER","PROJECT","CODE",
      "DRA/DT","DDEC/DT","RA-APP","DEC-APP","RA-MEAN", "DEC-MEAN",
      "RA-DATE","DEC-DATE","MJD_SRC","DATE-OBS",
...
#else
static char LtaFileVersion[]="COR30x2 LTA1.10 DAS2.01 HST00";
#endif
...
int put_antpar (CorrType *corr, char *p)
{
  ...
  int nant=30;
  char *p0 = p ;
  if(strstr(corr->version,"HST03") !=NULL) { nant=MAX_ANTS; }
  if(strstr(corr->version,"HST04") !=NULL) { nant=MAX_ANTS; }
...
{ int i,j,n ;
  ...
  for(i=0,n=0;i<corr->daspar.samplers;i++) if((j=corr->sampler[i].dpc + 1) >n)n=j;
  for (i=0; i < n ; i++)
  {
    ...
    { SamplerType *samp = corr->sampler + j ;
      if (samp->dpc == i)
      {// p+=sprintf(p,"%s %s 000\n", corr->antenna[samp->ant_id].name, corr->bandname[samp->band], samp->fft_id) ; // SSK fft_id made zero
        p+=sprintf(p,"%s %s %03d\n", corr->antenna[samp->ant_id].name, corr->bandname[samp->band], samp->fft_id) ;
        break ;
      }
    }
...
//  fprintf(stdout, "SAMPLER RANGE = %s\n", str);
  if(l<70)
...
{ int macs=0, i,j,k, inc_col,inc_row,pols ;
  ...
  for(i=0;i<32;i++)
	for(j=i;j<32;j++) {
        base[macs].card = 0;
        base[macs].chip = 0;
        base[macs].samp[0].fft_id = 4*i +2*DasCardId+1;
		base[macs].samp[1].fft_id = 4*j +2*DasCardId+1;
...
{ 
  ...
  fprintf(stdout, "==== >>>> INSIDE get_baselines\n");

//  corr->corrpar.macmode=0;  // added for some reason ??
//  fprintf(stdout, "MACMODE??? = %d\n", corr->corrpar.macmode);
...
{  /*  revised: 97july27/crs          */
  ...
  { Bufp += strlen(Bufp);
    ...
    sscanf(p+3,"%d", &id);
    if (id >= MAX_SAMPS) { fprintf(stderr,"ignoring BAD sampler %d\n",id) ; continue ; }
    sscanf(p+10,"%s %s", str1, str2 );
    /* antenna_name, band_name  to which it is connected */
    for (ant_id=0; ant_id < MAX_ANTS; ant_id++) if (str_cmp(str1, antenna[ant_id].name) == 0) break ;
    if (ant_id == MAX_ANTS ) continue ;
//    corr->corrpar.dpcmux=228; /// Hardcoded to 228, as it's value getting nullified, need to probe.
//    fprintf(stdout, "========>>>>>> DPCMUX = %d\n", corr->corrpar.dpcmux);
    fft_ids = dpc2fft(id,corr->corrpar.dpcmux,fft_id_list);
//    fprintf(stdout, "======>>>>> SAMP ID = %d FFT IDs = %d\n", id,fft_ids);
//    for(i=0;i<fft_ids;i++) fprintf(stdout, "SAMP ID = %d FFT_IDS = %d FFT_ID[%d] = %d\n",  id, fft_ids, i, fft_id_list[i]);
...
//	  fprintf(stdout, "%s %s BAND = %d FFT ID=%03d ANT_ID = %02d SAMP ID = %03d\n", str1, str2, band, fft_id, ant_id, id);
      if (band == MAX_BANDS)
      ...
      else{samp->dpc=MAX_SAMPS;fprintf(stdout, "SAMP-DPC=MAX");}
      antenna[ant_id].samp_id[band] = daspar->samplers ; /* local ref */
      daspar->samplers++;
    }
  }
//  fprintf(stdout, "No. of SAMS are %d\n", daspar->samplers);
...
void get_obs_mode(CorrType *corr, char *obs_mode)  
{ CorrParType *corrpar = &corr->corrpar;
  unsigned char macmode = corrpar->macmode;
  char     name[64];

  get_dpc_mode_name(corrpar->dpcmux,name);
  if(macmode == RRLL && !strcmp(name,"IndianPolar")){strcpy(obs_mode,"INDIAN_POLAR"); return ;}
  if(macmode == RRRL && !strcmp(name,"UsbPolar")){strcpy(obs_mode,"USB_POLAR"); return ;}
  if(macmode == RRRL && !strcmp(name,"LsbPolar")){strcpy(obs_mode,"LSB_POLAR"); return ;}
  if(macmode == RR__ && !strcmp(name,"UsbPolar")){strcpy(obs_mode,"USB_HIGH_RES"); return ;}
  if(macmode == RR__ && !strcmp(name,"LsbPolar")){strcpy(obs_mode,"LSB_HIGH_RES"); return ;}
  if(macmode == RRLL && !strcmp(name,"UsbCopy")){strcpy(obs_mode,"USB_COPY"); return ;}
  if(macmode == RRLL && !strcmp(name,"LsbCopy")){strcpy(obs_mode,"LSB_COPY"); return ;}
  if(macmode == RRLL && !strcmp(name,"AllU130")){strcpy(obs_mode,"ALL_U130"); return ;}
  if(macmode == RRLL && !strcmp(name,"AllU175")){strcpy(obs_mode,"ALL_U175"); return ;}
  if(macmode == RRLL && !strcmp(name,"AllL130")){strcpy(obs_mode,"ALL_L130"); return ;}
  if(macmode == RRLL && !strcmp(name,"AllL175")){strcpy(obs_mode,"ALL_L175"); return ;}
  strcpy(obs_mode,"CUSTOM");
  return;
}
  ...
  { p = Bufp;
    ...
    switch (n)
    {
      ...
      case Mac_mode : sscanf(p+10,"%s", str ) ;
                      val = search_str(str, mac_modes, MACMODES);
                      if(val != MACMODES) corrpar->macmode = val ;
//					  fprintf(stdout, "IN MACMODE CHECK %d\n", val);
                      break ;
...
//    case 1: sprintf(corr->version+24,"HST02 "); break; // HST01 changed to HST02 for cross polarization mode
...
{  
  ...
  { Bufp += strlen(Bufp);
    ...
    if (ant_id == MAX_ANTS ) continue ;
    fft_ids = dpc2fft(id, corr->corrpar.dpcmux,fft_id_list);
	fprintf(stdout, "ANT ID = %d FFT_IDS # = %d\n", ant_id, fft_ids);
	for(i=0,j=0;i<fft_ids;i++)
	{ int fft_id=fft_id_list[i];
	  if(DasCardId==DAS_CARD0 && (fft_id%4)/2) continue;
	  if(DasCardId==DAS_CARD1 && !((fft_id%4)/2)) continue;
...
{ char *p, str[32];
  ...
  { if (*Bufp == cont_char) continue;
    ...
    { p++; sscanf(p,"%s",str);
      ...
      { new_section=1; get_soft_sampler(f, corr);}
      if (str_cmp(str,"Corrsel.def") == 0)
      { new_section=1; get_soft_selection(f, corr);}
